/*
Copyright 2019 Rohith Jayawardene <info@appvia.io>

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package v1

import (
	"time"

	v1 "github.com/appvia/hub-apis/pkg/apis/config/v1"
	scheme "github.com/appvia/hub-apis/pkg/client/clientset/versioned/scheme"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	rest "k8s.io/client-go/rest"
)

// BindingListsGetter has a method to return a BindingListInterface.
// A group's client should implement this interface.
type BindingListsGetter interface {
	BindingLists(namespace string) BindingListInterface
}

// BindingListInterface has methods to work with BindingList resources.
type BindingListInterface interface {
	Create(*v1.BindingList) (*v1.BindingList, error)
	Update(*v1.BindingList) (*v1.BindingList, error)
	Delete(name string, options *metav1.DeleteOptions) error
	DeleteCollection(options *metav1.DeleteOptions, listOptions metav1.ListOptions) error
	Get(name string, options metav1.GetOptions) (*v1.BindingList, error)
	List(opts metav1.ListOptions) (*v1.BindingListList, error)
	Watch(opts metav1.ListOptions) (watch.Interface, error)
	Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.BindingList, err error)
	BindingListExpansion
}

// bindingLists implements BindingListInterface
type bindingLists struct {
	client rest.Interface
	ns     string
}

// newBindingLists returns a BindingLists
func newBindingLists(c *ConfigV1Client, namespace string) *bindingLists {
	return &bindingLists{
		client: c.RESTClient(),
		ns:     namespace,
	}
}

// Get takes name of the bindingList, and returns the corresponding bindingList object, and an error if there is any.
func (c *bindingLists) Get(name string, options metav1.GetOptions) (result *v1.BindingList, err error) {
	result = &v1.BindingList{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("bindinglists").
		Name(name).
		VersionedParams(&options, scheme.ParameterCodec).
		Do().
		Into(result)
	return
}

// List takes label and field selectors, and returns the list of BindingLists that match those selectors.
func (c *bindingLists) List(opts metav1.ListOptions) (result *v1.BindingListList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	result = &v1.BindingListList{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("bindinglists").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Do().
		Into(result)
	return
}

// Watch returns a watch.Interface that watches the requested bindingLists.
func (c *bindingLists) Watch(opts metav1.ListOptions) (watch.Interface, error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	opts.Watch = true
	return c.client.Get().
		Namespace(c.ns).
		Resource("bindinglists").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Watch()
}

// Create takes the representation of a bindingList and creates it.  Returns the server's representation of the bindingList, and an error, if there is any.
func (c *bindingLists) Create(bindingList *v1.BindingList) (result *v1.BindingList, err error) {
	result = &v1.BindingList{}
	err = c.client.Post().
		Namespace(c.ns).
		Resource("bindinglists").
		Body(bindingList).
		Do().
		Into(result)
	return
}

// Update takes the representation of a bindingList and updates it. Returns the server's representation of the bindingList, and an error, if there is any.
func (c *bindingLists) Update(bindingList *v1.BindingList) (result *v1.BindingList, err error) {
	result = &v1.BindingList{}
	err = c.client.Put().
		Namespace(c.ns).
		Resource("bindinglists").
		Name(bindingList.Name).
		Body(bindingList).
		Do().
		Into(result)
	return
}

// Delete takes name of the bindingList and deletes it. Returns an error if one occurs.
func (c *bindingLists) Delete(name string, options *metav1.DeleteOptions) error {
	return c.client.Delete().
		Namespace(c.ns).
		Resource("bindinglists").
		Name(name).
		Body(options).
		Do().
		Error()
}

// DeleteCollection deletes a collection of objects.
func (c *bindingLists) DeleteCollection(options *metav1.DeleteOptions, listOptions metav1.ListOptions) error {
	var timeout time.Duration
	if listOptions.TimeoutSeconds != nil {
		timeout = time.Duration(*listOptions.TimeoutSeconds) * time.Second
	}
	return c.client.Delete().
		Namespace(c.ns).
		Resource("bindinglists").
		VersionedParams(&listOptions, scheme.ParameterCodec).
		Timeout(timeout).
		Body(options).
		Do().
		Error()
}

// Patch applies the patch and returns the patched bindingList.
func (c *bindingLists) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.BindingList, err error) {
	result = &v1.BindingList{}
	err = c.client.Patch(pt).
		Namespace(c.ns).
		Resource("bindinglists").
		SubResource(subresources...).
		Name(name).
		Body(data).
		Do().
		Into(result)
	return
}
