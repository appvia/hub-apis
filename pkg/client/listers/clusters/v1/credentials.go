...
type CredentialsLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Credentials, err error)
...
// List lists all Credentialses in the indexer.
func (s *credentialsLister) List(selector labels.Selector) (ret []*v1.Credentials, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Credentials))
	})
	return ret, err
}
...
type CredentialsNamespaceLister interface {
	...
	Get(name string) (*v1.Credentials, error)
...
// List lists all Credentialses in the indexer for a given namespace.
func (s credentialsNamespaceLister) List(selector labels.Selector) (ret []*v1.Credentials, err error) {
...
// Get retrieves the Credentials from the indexer for a given namespace and name.
func (s credentialsNamespaceLister) Get(name string) (*v1.Credentials, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("credentials"), name)
	}
	return obj.(*v1.Credentials), nil
}
